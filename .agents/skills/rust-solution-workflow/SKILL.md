---
name: rust-solution-workflow
description: Build or change Rust solutions through specification, adversarial test-first development, implementation, verification, review, and improvement with a mandatory user approval gate after every phase. Use for Rust exercises, challenge solutions, small services, libraries, and focused repository changes that need hidden-test resilience, implicit-contract analysis, or architecture explained for a C# background.
---

# Rust Solution Workflow

Deliver Rust work through this sequence:

`specification -> tests -> agent implementation -> tests -> review -> improve`

The user is the reviewer at every phase. Never treat invocation of this skill as approval to cross all gates.
...
Read and follow each selected skill when its phase begins. Do not load every related skill preemptively.
...
Separate the result into four categories and carry them through every review packet:
...
Do not silently turn an assumption into architecture. If an ambiguity materially changes externally visible behavior, memory growth, compatibility, or failure semantics, raise it at the current gate. When a reversible local choice is safe, state the choice, rationale, and corresponding test.
...
Maintain these default invariants unless the approved specification says otherwise:
...
1. Give the user a compact review packet containing the artifacts produced, decisions made, evidence gathered, and unresolved tradeoffs.
2. State exactly what the next phase would do and which files it may change.
3. End the turn and wait for explicit approval, requested revisions, or cancellation.

Stay in the current phase when the user requests changes. Reissue the updated review packet after making them. Do not interpret silence, the original task, or approval of an earlier phase as approval of a later phase.
...
**Gate:** Ask the user to approve or revise the specification before creating or editing implementation files.

## Phase 2: Tests

Turn the approved acceptance criteria into behavior-focused tests before production implementation.

- Cover the normal path, boundary cases, and meaningful error paths.
- Build tests from the approved state transitions and invariants, then combine dimensions likely to interact instead of testing every concern only in isolation.
...
The review packet must include the test names, files changed, mapped acceptance criteria, and the observed red-state evidence.
Also identify which hidden-test families are proven, which remain untested, and why the selected cases generalize beyond their concrete values.

**Gate:** Ask the user to approve or revise the tests before implementing production behavior.

## Phase 3: Agent implementation

Implement the smallest clear Rust change that satisfies the approved tests.
...
- Preserve public and protocol compatibility unless an approved specification explicitly changes it.

Use targeted red-green iterations while implementing. Do not expand scope or perform unrelated cleanup. Explain non-obvious Rust choices using concise C# comparisons in the review packet, not tutorial comments in otherwise clear code.

The review packet must include changed files, important design choices, targeted test evidence, and any deviation from the approved specification.

**Gate:** Ask the user to approve or revise the implementation before full verification.
...
If formatting fails, report it and request permission to run the mutating `cargo fmt` when that action was not already authorized. Do not hide warnings, skip failing tests, or claim success from partial output. Add an end-to-end or boundary check when the acceptance criteria are not fully proven by the automated suite.
...
The review packet must list every command, its result, test counts where available, and any remaining verification gap.

**Gate:** Ask the user to approve the verification result before code review.

## Phase 5: Review

Review the approved change without editing it. Evaluate:
...
The review packet must contain the verdict and a proposed improvement set. Do not apply findings yet.

**Gate:** Ask the user which findings to approve, reject, or revise before changing code.

## Phase 6: Improve

Apply only the approved review findings. Keep behavior changes test-first and keep refactors behavior-preserving.

- Make the narrowest improvements that resolve the findings.
- Add a regression test for every approved correctness or state-invariant defect before fixing it.
...
Do not commit, push, publish, deploy, or mutate external systems unless the user separately requested that action or repository instructions require it.

**Gate:** Present the completed result for the user's final review and wait for acceptance or another improvement cycle.

## Stop conditions

Stop at the current gate when requirements conflict, a test fails for an unexpected reason, verification tooling is unavailable, user-owned changes overlap unsafely, or the next action needs new authority. Provide the evidence and the smallest decision needed from the user.
