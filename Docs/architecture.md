...
The Codex CLI adapter invokes `codex exec` directly with JSONL events, a strict output schema, an ephemeral session, and a read-only sandbox for structured tasks. Brief text travels through stdin rather than process arguments. The OpenCode adapter lazily manages one loopback `opencode serve` child per cloned provider client, discovers its ephemeral listening port from the server announcement, creates one session per capability call, routes each request through `x-opencode-directory`, and uses the server's native JSON Schema message format for structured calls. It also filters the global SSE event stream by session before translating safe progress into provider-neutral activity; server credentials are inherited from `OPENCODE_SERVER_PASSWORD` and `OPENCODE_SERVER_USERNAME` without entering logs. Codex and OpenCode initial analysis have a twenty-minute hard deadline; research, auto-answer, and documentation instead reset a twenty-minute inactivity timer on provider activity. OpenCode server startup and session creation each have a separate twenty-minute deadline. The local adapter uses a proxy-free `reqwest` client against a numeric loopback `/v1` endpoint, streams OpenAI-compatible chat completions, and applies the same JSON schemas and response validators. Valid local events reset a configurable inactivity window (1200 seconds by default, 1–1200 via `--local-inactivity-timeout-secs`). Local research and all auto-answer stages have no total inference deadline, but runtime startup remains bounded by twenty minutes. Local analysis and documentation retain their twenty-minute hard limit including startup. All provider operation defaults share one twenty-minute CLI constant; short network connection, health-probe, and cleanup limits remain separate. Cancellation, response-size limits, and tool-round bounds apply throughout. Tests use deterministic subprocess and loopback HTTP fixtures and never require a live model.
