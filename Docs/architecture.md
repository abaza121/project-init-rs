...
The Codex CLI adapter invokes `codex exec` directly with JSONL events, a strict output schema, an ephemeral session, and a read-only sandbox for structured tasks. Brief text travels through stdin rather than process arguments. Codex and OpenCode initial analysis have a five-minute hard deadline; research and documentation instead reset a five-minute inactivity timer on provider output. The local adapter uses a proxy-free `reqwest` client against a numeric loopback `/v1` endpoint, streams OpenAI-compatible chat completions, and applies the same JSON schemas and response validators. Valid local events reset a configurable inactivity window (60 seconds by default, 1–600 via `--local-inactivity-timeout-secs`). Local research and all auto-answer stages have no total inference deadline, but runtime startup remains bounded by twenty minutes. Local analysis and documentation retain their twenty-minute hard limit including startup. Cancellation, response-size limits, and tool-round bounds apply throughout. Tests use deterministic subprocess and loopback HTTP fixtures and never require a live model.
