...
`new` runs an isolated, schema-constrained Codex CLI analysis by default. When attached to a terminal it shows a spinner and a rolling, scrollable activity timeline; `Esc` cancels immediately. The run times out after twenty minutes, and failure, cancellation, or invalid structured output creates no project state. The application resolves a native Codex executable from `PATH`, including the binary bundled by an npm installation. Set `CODEX_BIN` to the full native executable path (`codex.exe` on Windows) to override discovery, or pass `--offline` to use the conservative deterministic analyzer explicitly.

All program-launched Codex tasks pass `--skip-git-repo-check`, including the first analysis, so extracted builds and generated data directories do not need a Git repository or a saved directory-trust setting. Analysis and research retain the read-only sandbox; document generation retains its staging-directory workspace-write sandbox. Other Codex permission checks and authentication requirements remain in effect, and user configuration is not changed. Structured-task failures include a bounded, sanitized excerpt of Codex stderr when available.

Pass the global `--provider opencode` to run the same analysis, cited research, automatic-answer, generation, repair, and workbench flows through the authenticated OpenCode profile. Project Init lazily starts one loopback `opencode serve` process per provider client, creates an isolated session per operation, sends structured calls with OpenCode's native JSON Schema format, and routes each request to its scoped working or staging directory. Existing output validation, cancellation, timeout, and staging-adoption boundaries remain in force. Set `OPENCODE_BIN` to the full native executable path (`opencode.exe` on Windows) to override `PATH` discovery. Project Init never passes OpenCode's permission auto-approval flag: your OpenCode profile remains responsible for models, agents, plugins, authentication, and tool permissions. The server inherits `OPENCODE_SERVER_PASSWORD` and `OPENCODE_SERVER_USERNAME` when configured. See the [OpenCode CLI reference](https://dev.opencode.ai/docs/cli/) and [OpenCode server API](https://dev.opencode.ai/docs/server/).

Pass the global `--provider local` plus a model directory, versioned `mistral.rs` image, and `cpu` or `cuda` device to use local HTTP inference as a secondary option. GGUF remains the default and requires the pinned Gemma file; pass `--local-format plain` (or `tensor`) for a native safetensors directory containing `config.json` and at least one `.safetensors` file. Project Init verifies a healthy loopback service or launches a hardened Docker container, then uses the same provider for analysis, cited DuckDuckGo research, automatic answers, generation, repair, and workbench resume. Local document generation receives only three exact allowlisted application tools; it never receives shell, Python, or generic host filesystem access. Local operations use an activity-reset inactivity timer (twenty minutes by default; shorten with `--local-inactivity-timeout-secs 300`, range 1–1200 seconds). Research and auto-answer can continue beyond twenty minutes while valid model activity arrives. Startup remains bounded, and initial analysis and documentation retain their twenty-minute total cap. See [`Docs/local-inference-setup.md`](Docs/local-inference-setup.md) for the model setup, CPU/CUDA configuration, tuning, security boundary, and smoke commands. Web research still needs network access, and provider failures never trigger an automatic fallback.
...
The workbench provides Overview, Questions, Findings, Requirements, and Activity sections plus a contextual composer. Use Left/Right to change section, Up/Down to select a question or scroll a read-only panel, `Tab` to move focus, and `Enter` to answer the selected open question. Overview, Findings, Requirements, and Activity also support `Page Up`/`Page Down` and `Home`/`End` while outside the composer, and each remembers its own scroll position. Overview includes the complete project timeline. `Ctrl+Enter` inserts a newline when the terminal reports that key combination. `/answer <question-id> <text>` targets a question explicitly, `/ask [prompt]` opens structured question capture with visible conservative defaults, and `/threshold <1-125>` changes the persisted consequential-question threshold. The standalone `answer` command remains available for scripts and non-interactive use.
...
Codex and OpenCode research, auto-answer planning/workers/judgment, generation, and repair use a twenty-minute inactivity timeout: Codex stdout/stderr activity or OpenCode session events reset the timer, so productive operations can run longer than twenty minutes. Cancellation remains available throughout. Initial brief analysis uses a twenty-minute hard deadline; OpenCode server startup and session creation also each allow twenty minutes. Local inference retains the hard-versus-inactivity behavior described above. Short network connection, health-probe, and cleanup limits remain separate from provider operation timeouts.
