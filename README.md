...
Codex and OpenCode research, auto-answer planning/workers/judgment, generation, and repair use a five-minute inactivity timeout: stdout or stderr activity resets the timer, so productive operations can run longer than five minutes. Cancellation remains available throughout. Initial brief analysis retains its five-minute total deadline, and local inference retains its separate limits described above.
